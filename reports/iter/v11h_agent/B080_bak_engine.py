...
class CompoundConfig:
    ...
    risk_cap_pct: float = 0.0024  # KEEP A408 (A409 0.00235 REVERT)
...
def run_compound(df, signals, name, config=None) -> CompoundResult:
    ...
    size_boost = signals["size_boost"].to_numpy(np.float64) if "size_boost" in signals else np.ones(n)
    ...
    eq0 = float(equity)  # KEEP A34: baseline for equity-scaled risk
    ...
    for i in range(n):
        ...
        if pending != 0 and pos_side == 0:
            ...
            # R16: cap initial risk distance
            if cfg.risk_cap_pct > 0 and not np.isnan(pos_sl):
                if pos_side == 1:
                    cap = fill * (1.0 - cfg.risk_cap_pct)
                    pos_sl = max(pos_sl, cap)
                else:
                    cap = fill * (1.0 + cfg.risk_cap_pct)
                    pos_sl = min(pos_sl, cap)
            boost = float(size_boost[i]) if not np.isnan(size_boost[i]) else 1.0
            # KEEP A33+A34: equity-scaled risk target 8%->25%
            lev_cap = equity * cfg.leverage * size_mult * 1.5 * boost
            pos_risk = abs(fill - pos_sl) if not np.isnan(pos_sl) else fill * 0.01
        ...
        if pos_side != 0:
            ...
            # R22: breakeven stop after +be_at_r
            if cfg.be_at_r > 0 and (not pos_be_done) and pos_risk > 0:
                move_be = (high[i] - pos_entry) if pos_side == 1 else (pos_entry - low[i])
                if move_be >= cfg.be_at_r * pos_risk:
                    pos_sl = pos_entry
                    pos_be_done = True
            ...
            # R13 grace: suppress SL (not liq) early in trade
            if cfg.sl_grace_bars > 0 and (i - pos_i) < cfg.sl_grace_bars:
                hit_sl = False
            ...
            if reason is not None:
                ...
                # KEEP A147-A153 staged scale-outs; A154 raises cap to 7
                do_partial = (
                    cfg.partial_trail_frac > 0
                    and cfg.partial_trail_frac < 1
                    and pos_scale_n < cfg.trail_scale_max
                    and reason == "sl"
                    and in_profit
                )
                if do_partial:
                    if pos_scale_n == 0:
                        ...
                        close_frac = 0.02 if pos_boost >= 1.25 else 0.08  # KEEP A389-A392
                    else:
                        close_frac = 0.015  # KEEP B074 (agent manual)
                else:
                    close_frac = 1.0
                ...
                if do_partial:
                    ...
                    pos_scale_n += 1
                    # KEEP A70: keep trail SL (do not snap to BE)
                else:
                    pos_side = 0
