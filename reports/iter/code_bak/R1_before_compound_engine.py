...
class CompoundConfig:
    ...
    # code-level trail unlock: after +unlock_r, multiply trail distance
...
def run_compound(df, signals, name, config=None) -> CompoundResult:
    ...
    for i in range(n):
        ...
        if pending != 0 and pos_side == 0:
            ...
            pos_notional = equity * cfg.leverage
            pos_peak = fill
        ...
        if pos_side != 0:
            ...
            if cfg.trail_unlock_r > 0 and pos_risk > 0 and (not np.isnan(tr)):
                move = (high[i] - pos_entry) if pos_side == 1 else (pos_entry - low[i])
                if move >= cfg.trail_unlock_r * pos_risk:
            ...
            if reason is not None:
                ...
                net = (-equity * 0.99) if reason == "liquidation" else (gross - fee)
